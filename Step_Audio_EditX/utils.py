...
def volumn_adjust(audio16bit_torch, sr, volumn_ratio):
    ...
    if not enable_sox:
        # 直接通过数值乘法调整音量
        audio_changed = audio16bit_torch * volumn_ratio
        
        # 防止溢出，进行归一化处理（保持在 [-1, 1] 范围内）
        max_val = torch.max(torch.abs(audio_changed))
        if max_val > 0.999:
            audio_changed = audio_changed / max_val * 0.999
    else:
        #创建临时文件
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=True) as temp_in, tempfile.NamedTemporaryFile(suffix=".wav", delete=True) as temp_out:
            # 保存输入音频到临时文件
...
def speech_adjust(audio16bit_torch, sr, speed_ratio):
    ...
    if not enable_sox:
        ...
        # 计算新的长度
        original_length = audio16bit_torch.shape[1]
        new_length = max(1, int(original_length / speed_ratio))  # 确保至少有1个样本
        
        # 使用插值调整时间轴
        audio_stretched = torch.nn.functional.interpolate(
            audio16bit_torch.unsqueeze(0),  # 添加批次维度
            size=new_length,
            mode='linear',
            align_corners=True
        ).squeeze(0)  # 移除批次维度
        ...
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=True) as temp_in, tempfile.NamedTemporaryFile(suffix=".wav", delete=True) as temp_out:
            ...
            tfm.build_file(temp_in.name, temp_out.name)
            # 读取处理后的音频
            audio_stretched, _ = torchaudio.load(temp_out.name)
