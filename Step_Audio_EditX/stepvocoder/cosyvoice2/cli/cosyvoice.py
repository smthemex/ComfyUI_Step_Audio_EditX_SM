...
from ..embedding.dual_codebook import DualCodebookEmbedding
from ..transformer.upsample_encoder_v2 import UpsampleConformerEncoderV2
from ..flow.flow_matching import CausalConditionalCFM
from ..flow.decoder_dit import DiT
...
stepvocoder_parent = os.path.dirname(parent_dir)
if stepvocoder_parent not in sys.path:
    sys.path.insert(0, stepvocoder_parent)
...
class CosyVoice:
    def __init__(self, 
        ...
        # 手动构建 flow 模型
        input_embedding = DualCodebookEmbedding(
        ...
        flow_state_dict = torch.load(f"{model_dir}/flow.pt", map_location='cpu',weights_only=False)
        hift_state_dict = torch.load(f"{model_dir}/hift.pt", map_location='cpu',weights_only=False)
        flow.load_state_dict(flow_state_dict,strict = True)
        flow = flow.eval()
        hift.load_state_dict(hift_state_dict,strict = True)
        hift = hift.eval()
        del flow_state_dict, hift_state_dict
    ...
    def to(self, device):
         self.cosy_impl.to(device=torch.device(device))
    # Just proxy
    def token2wav_nonstream(self,
